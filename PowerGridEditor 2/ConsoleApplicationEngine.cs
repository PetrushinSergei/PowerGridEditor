...
namespace PowerGridEditor
{
    ...
    internal sealed class ConsoleApplicationEngine
    {
        ...
        {
            int L = m;
            // Оставляем стандартное заполнение массивов для алгоритма Alpha
            for (int i = 1; i <= L; i++)
            {
                var row = tokRows[i - 1];
                na[i] = row.Start;
            ...
            net2.AppendLine();
            net2.AppendLine("           Токи ветвей (в Амперах, А)");
            net2.AppendLine(" Ветвь Нач.   Кон.     I_нач (А)    I_кон (А)");
            net2.AppendLine("-------------------------------------------------");

            // Множитель для точного перевода фазных величин в линейный ток Растра
            // 1000.0 / Math.Sqrt(3) ≈ 577.35026919
            double toAmperes = 1000.0 / Math.Sqrt(3.0);

            for (int i = 1; i <= L; i++)
            {
                int i1 = na[i]; // Индекс узла начала
                int i2 = ka[i]; // Индекс узла конца

                // 1. Квадраты модулей напряжений на краях ветви
                double mv1 = va[i1] * va[i1] + vr[i1] * vr[i1];
                double mv2 = va[i2] * va[i2] + vr[i2] * vr[i2];

                // 2. Рассчитываем чистые продольные токи (ток через R + jX)
                double i1a_prod = (va[i1] * gr[i] - vr[i1] * bx[i]) / (kt[i] * kt[i]) - (va[i2] * gr[i] - vr[i2] * bx[i]) / kt[i];
                double i1r_prod = (va[i1] * bx[i] + vr[i1] * gr[i]) / (kt[i] * kt[i]) - (va[i2] * bx[i] + vr[i2] * gr[i]) / kt[i];

                double i2a_prod = (va[i1] * gr[i] - vr[i1] * bx[i]) / kt[i] - (va[i2] * gr[i] - vr[i2] * bx[i]);
                double i2r_prod = (va[i1] * bx[i] + vr[i1] * gr[i]) / kt[i] - (va[i2] * bx[i] + vr[i2] * gr[i]);

                // 3. Учитываем емкостные токи шунтов линии (B/2) на каждом конце ветви
                double i1a_sh = (va[i1] * gy[i] - vr[i1] * by[i]) / 2.0;
                double i1r_sh = (va[i1] * by[i] + vr[i1] * gy[i]) / 2.0;

                double i2a_sh = (va[i2] * gy[i] - vr[i2] * by[i]) / 2.0;
                double i2r_sh = (va[i2] * by[i] + vr[i2] * gy[i]) / 2.0;

                // 4. Полный ток на шинах начала и конца с учетом знаков направлений П-схемы замещения
                double i1a_full = i1a_prod + i1a_sh;
                double i1r_full = i1r_prod + i1r_sh;

                double i2a_full = i2a_prod - i2a_sh;
                double i2r_full = i2r_prod - i2r_sh;

                // 5. Вычисляем итоговые модули токов в Амперах (с делением на корень из 3)
                double i1_amps = Math.Sqrt(i1a_full * i1a_full + i1r_full * i1r_full) * toAmperes;
                double i2_amps = Math.Sqrt(i2a_full * i2a_full + i2r_full * i2r_full) * toAmperes;

                // Вывод строго двух колонок токов, как вы просили
                net2.AppendLine($"{nn[i1],5}{nn[i2],7} {F2(i1_amps),13} {F2(i2_amps),12}");
            }

            // --- Математический блок алгоритма Alpha (не изменяется для корректности матриц) ---
            for (int i = 1; i <= L; i++)
                for (int j = 0; j <= n; j++)
        ...
        private struct TokRow
        {
            public int Start;
            public int End;
            public double I1a; // Активный ток начала (кА)
            public double I1r; // Реактивный ток начала (кА)
            public double I2a; // Активный ток конца (кА)
            public double I2r; // Реактивный ток конца (кА)
            public double R;
        }
    }
}